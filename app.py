...
if not DB_PATH.exists():
    with st.spinner("First run — building database..."):
        if not CSV_PATH.exists():
            subprocess.run([sys.executable, "data/generate_data.py"], cwd=PROJECT_ROOT, check=True)
        subprocess.run([sys.executable, "-m", "src.etl"], cwd=PROJECT_ROOT, check=True)

from src import queries

st.set_page_config(page_title="Personal Finance Analytics", page_icon="💰", layout="wide")
st.markdown("""<style>
...
with st.sidebar:
    ...
    st.caption("Synthetic bank data. End-to-end analytics: CSV, Python ETL, SQLite, interactive dashboard.")
    st.caption("Code: [github.com/adi0x/finance-tracker](https://github.com/adi0x/finance-tracker)")
...
col3.metric("Net Savings", f"${kpis['net_savings']:,.0f}",
            delta="Positive" if kpis['net_savings'] >= 0 else "Overspending",
...
with left:
    ...
    if not monthly.empty:
        fig = px.bar(monthly, x="month_label", y=["income", "spending"], barmode="group",
                     labels={"month_label": "Month", "value": "Amount ($)", "variable": ""},
                     color_discrete_map={"income": "#10b981", "spending": "#ef4444"})
        fig.update_layout(margin=dict(l=0, r=0, t=10, b=0), plot_bgcolor="white", height=340,
...
with right:
    ...
    if not cats.empty:
        fig = px.pie(cats, names="category", values="total_spent", hole=0.55)
...
if category_choices:
    ...
    if not trend.empty:
        ...
        with c1:
            fig = px.line(trend, x="month_label", y="spent", markers=True,
                          labels={"month_label": "Month", "spent": "Spent ($)"})
            fig.update_traces(line_color="#6366f1", line_width=3, marker_size=8)
            fig.update_layout(margin=dict(l=0, r=0, t=10, b=0), plot_bgcolor="white", height=280)
            fig.update_xaxes(showgrid=False); fig.update_yaxes(gridcolor="#f3f4f6")
            st.plotly_chart(fig, use_container_width=True)
        with c2:
...
with left:
    ...
    if not merchants.empty:
        st.dataframe(merchants.rename(columns={"merchant": "Merchant", "category": "Category",
                                               "visits": "Visits", "total_spent": "Total Spent ($)"}),
...
with right:
    ...
    if not anomalies_df.empty:
        st.dataframe(anomalies_df.rename(columns={"transaction_date": "Date", "description": "Description",
                                                  "category": "Category", "amount": "Amount ($)", "z_score": "Z-Score"}),
                     use_container_width=True, hide_index=True)
    else:
        st.info("No anomalies detected in this range.")

st.divider()
st.caption("Built by Adithi Koppula · Python · SQLite · Streamlit · Plotly · [GitHub](https://github.com/adi0x/finance-tracker)")
